program main
  ...
  integer, parameter :: procgrid1 = 2, procgrid2 = 3
  ...
  call matd_create_scalapack_gellan( &
    m, dim1, dim2, block_size1, block_size2, procgrid1, procgrid2, &
    mpi_comm_world, X, ibegin, iend &
  )
  call matd_fence(m)
  ...
  ! したがって要素分としては24 / 2 = 12
  ! インデックスが進む。
  ! またマップ情報分(4バイト整数*(6+5)) 11 / 2 + 1 = 6
  ! インデックスが進み、計18インデックスが進む。
  do i = 0, 5
    if (myrank == i) then
      print *, iend - ibegin
    endif
